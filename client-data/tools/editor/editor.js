...
(function editor() {
  ...
  var resize = [
    "<span style='margin-top:-3px;margin-left:-3px;opacity:.5;'>" +
      `<img style="pointer-events:none;" draggable="false" src='data:image/svg+xml;utf8,` +
      img1 +
      `' >` +
      "</span>",
  ];
...
int main() {
  ...
  function change_editor(i) {
    ...
    document.getElementById("wb-editor-curmode").textContent =
      mode[modeIndex].name;
    if (i != 0) {
      var sp = document.getElementById("wb-spell-check");
      sp.style.color = "";
      sp.style.display = "none";
      document.getElementById("wb-code-template").style.display =
        "inline-block";
  ...
  var change = function (cMirror, obj) {
    ...
    if (spellCheck) setLines(changeLock ? -1000000 : obj.from.line);
  };
  ...
  function init_texteditor() {
    ...
    function getExampleRef() {
      ...
      if (firepad_config.childref) ref = ref.child(firepad_config.childref);
  ...
  function init_window() {
    document
      .getElementById("wb-options")
      .addEventListener("click", toggleOptionsBar);
    document
      .getElementById("wb-close-options")
      .addEventListener("click", toggleOptionsBar);
    document
      .getElementById("wb-textoptions")
      .addEventListener("click", toggleToolbar);
    document
      .getElementById("wb-close-edit")
      .addEventListener("click", toggleToolbar);
    document.getElementById("wb-editor-run").addEventListener("click", run);
    document
      .getElementById("wb-editor-live")
      .addEventListener("click", live_eval);
    ...
    document
      .getElementById("wb-spell-check")
      .addEventListener("click", toggleSpelling);
    document
      .getElementById("wb-code-template")
      .addEventListener("click", openInsertDialog);
    document
      .getElementById("wb-ed-insert-template")
      .addEventListener("click", insertTemplate);
    document
      .getElementById("wb-code-template")
      .addEventListener("click", openInsertDialog);
    document
    ...
    document.getElementById("wb-text").addEventListener(
      "click",
      function () {
        change_editor(0);
      },
    ...
    document.getElementById("wb-js").addEventListener(
      "click",
      function () {
        change_editor(1);
      },
    ...
    document.getElementById("wb-python").addEventListener(
      "click",
      function () {
        change_editor(2);
      },
    ...
    document.getElementById("wb-cpp").addEventListener(
      "click",
      function () {
        change_editor(3);
      },
    ...
    function Resize(e) {
      ...
      win.h = Math.max(
        ...
        e.pageY -
          container.offsetTop -
          (40 + (options_open ? 45 : 0) + (toolbar_open ? 30 : 0))
      );
    ...
    function stopResize(e) {
      $(container).css({
        "-webkit-user-select": "" /* and add the CSS class here instead */,
        "user-select": "",
      });
  ...
  function unmax_win() {
    ...
    container.style.left =
      (win.l
        ? win.l - document.documentElement.scrollLeft
        : 65 + document.documentElement.scrollLeft) + "px";
    container.style.top =
      (win.t
        ? win.t - document.documentElement.scrollTop
        : 19 + document.documentElement.scrollTop) + "px";
  ...
  function max_win() {
    ...
    container.style.paddingBottom =
      45 + (options_open ? 45 : 0) + (toolbar_open ? 30 : 0) + "px";
    var elem = document.getElementById("wb-editor");
    win.w = $(elem).width();
    win.h = $(elem).height();
  ...
  function loadLanguage(done, lang) {
    ...
    xhr_aff.open(
      "GET",
      "https://rawgit.com/ropensci/hunspell/master/inst/dict/" + lang + ".aff",
      true
    );
    xhr_aff.onload = function () {
      if (xhr_aff.readyState === 4 && xhr_aff.status === 200) {
        ...
        xhr_dic.open(
          "GET",
          "https://rawgit.com/ropensci/hunspell/master/inst/dict/" +
            lang +
            ".dic",
          true
        );
        xhr_dic.onload = function () {
          if (xhr_dic.readyState === 4 && xhr_dic.status === 200) {
            //console.log('dic loaded');
              ...
              new Typo(lang, xhr_aff.responseText, xhr_dic.responseText, {
                platform: "any",
              })
            );
          } else {
      ...
      token: function (stream) {
        ...
        if (
          !(
            linenum == c.line &&
            stream.column() < c.ch &&
            c.ch - (stream.column() + word.length) < 1
          )
        )
