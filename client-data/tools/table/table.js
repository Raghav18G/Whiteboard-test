...
(function Table() {
  ...
  const createTable = (e, fieldData) => {
    ...
    crossDiv.classList.add("table-drag");
...
const addInputElement = (row) => {
  ...
  input.addEventListener("change", (e) => {
    ...
    input.style.width =
      Math.min(Math.min(inputWidth, table.offsetWidth), 450) + "px";
    console.log(input.style.width, "input.style.width");
    if (input.style.width === "450px") {
      input.classList.remove("add-wrap");
    }
    table
      .querySelectorAll(`td:nth-child(${colIndex + 1}) textArea`)
