...
(function calculator() {
  ...
  function diff(state) {
    ...
    let prevExpressions = {};
    let ids = new Set();
    olist.forEach((expression) => {
      prevExpressions[expression.id] = expression;
      ids.add(expression.id);
    });
    nlist.forEach((expression) => {
      if (
        JSON.stringify(expression) ==
        JSON.stringify(prevExpressions[expression.id])
      ) {
  ...
  function getLists(
    { expressions: { list: olist = [] } = {} } = {},
    { expressions: { list: nlist = [] } = {} }
  ) {
    return [olist, nlist];
  }
