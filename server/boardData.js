...
BoardData.prototype.update = function (id, message) {
  ...
  if (Array.isArray(id)) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      if (typeof elem == "object") {
        ...
        for (var j in newData[i]) {
          var sz;
          if (!(sz = this.isValid(message, "bulk-update", j, newData[i][j])))
            return;
          diff[i] += sz - memorySizeOf(data[j]);
          if (data[j]) oldData[i][j] = data[j];
        }
        save = true;
    ...
    if (save) {
      if (undo) this.updateActionList(ids, gid, newData, oldData, diff);
      for (var i = 0; i < ids.length; i++) {
        var elem = this.elements[ids[i]];
        if (typeof elem == "object") {
          var data = elem.data;
    ...
    for (var i in newData) {
      ...
      if (data[i]) oldData[i] = data[i];
    }
    if (this.size + diff > config.MAX_BOARD_BYTES) return;
    if (undo) this.updateActionList(id, gid, newData, oldData, diff);
    for (var i in newData) {
      data[i] = newData[i];
    }
    elem.size += diff;
          ...
          for (var i = 0; i < lastEvent.id.length; i++) {
            ...
            if (typeof elem == "object") {
              ...
              for (var j in lastEvent.oldData[i]) {
                this.elements[lastEvent.id[i]].data[j] =
                  lastEvent.oldData[i][j];
              }
            }
          }
        } else {
 ...
 */
BoardData.prototype.updateActionList = function (
  id,
  gid,
  newData,
  oldData,
